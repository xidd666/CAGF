...
class hubert(nn.Module):            
    ...
    def forward(self, input_ids, attention_mask):        
        ...
        hidden_states = raw_output.last_hidden_state    # Shape is [batch_size, seq_length, 1024]
        ...
        for batch in range(hidden_states.shape[0]):
            ...
            layer = 0
            while layer<24:  # chinese-hubert-large: 24 layers
                try:
            ...
            avg_feature = torch.mean(hidden_states[batch][:padding_idx],0) #Shape is [1024]
            features.append(avg_feature)
            
        features = torch.stack(features,0).to(device) #Shape is [batch_size, 1024]
